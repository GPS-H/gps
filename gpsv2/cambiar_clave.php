...
<table border="0" class="table table-bordered">
  <tr class="success">
    <td>
    	<center><strong>
        	<h3><img src="img/usuario.png" class="img-circle img-polaroid" width="50" height="65"> 
  ...
  <tr>
	...
	if(!empty($_POST['c1']) and !empty($_POST['c2']) and !empty($_POST['contra'])){
		if($_POST['c1']==$_POST['c2']){
			...
			$can=mysql_query("SELECT * FROM usuario WHERE usuario='".$usuario."' and contrasenia='".$contra."'");
			if($dato=mysql_fetch_array($can)){
				$cnueva=limpiar($_POST['c2']);
				$sql="Update usuario Set contrasenia='$cnueva' Where usuario='$usuario'";
				mysql_query($sql);
